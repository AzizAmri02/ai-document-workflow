...
class DocumentService:
    ...
    def update_status(self, document_id: str, user: User, new_status: str, comment: str | None) -> Document:
        document = self.get_document(document_id, user)

        try:
            target_status = DocumentStatus(new_status)
        except ValueError as exc:
            raise HTTPException(status_code=status.HTTP_422_UNPROCESSABLE_ENTITY, detail="Invalid status") from exc
        ...
        if target_status not in allowed:
            raise HTTPException(
                ...
                detail=f"Cannot transition from {document.status.value} to {target_status.value}",
            )

        if target_status in {DocumentStatus.approved, DocumentStatus.rejected}:
            if user.role != UserRole.reviewer:
                raise HTTPException(status_code=status.HTTP_403_FORBIDDEN, detail="Reviewer role required")

        if target_status in {DocumentStatus.pending_review, DocumentStatus.draft}:
            if document.owner_id != user.id:
                raise HTTPException(
                    status_code=status.HTTP_403_FORBIDDEN,
                    detail="Only document owners can perform this action",
                )

        if target_status == DocumentStatus.rejected and not comment:
            raise HTTPException(status_code=status.HTTP_422_UNPROCESSABLE_ENTITY, detail="Rejection comment is required")

        self.repo.update_status(document, target_status, user, comment)
        return document

    def get_status_history(self, document_id: str, user: User):
        self.get_document(document_id, user)
        return self.repo.list_status_history(document_id)
